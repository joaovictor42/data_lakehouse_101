...
- Se `pipeline-init` falhou (ex: erro transitório de rede/timing na primeira subida), rode manualmente: `docker exec lakehouse-jupyter python3 /home/jovyan/trino-init/construir_pipeline.py` — isso constrói tudo do mesmo jeito, só que na mão (sem nenhum arquivo pra sujar, é só um script).
