...
### `lakehouse_kit`: além de ler via Trino

`lh.query(...)` cobre a leitura — mas ler é só metade da arquitetura medalhão. `lakehouse_kit` também tem o par que fecha o ciclo, pensado pra ser simples o bastante pro aluno construir sua própria camada sem aprender a sintaxe do Hive/Trino pra tabela externa:

- `lh.read_table(layer, tabela)` — lê o Parquet **direto do MinIO** com pandas (`pd.read_parquet(..., storage_options=...)`), sem passar pelo Trino. Diferença importante pra `lh.query(...)`: este último só enxerga o que já foi cadastrado no catálogo (Hive Metastore); `read_table` lê o arquivo bruto, mesmo que ele nunca tenha sido cadastrado em lugar nenhum.
- `lh.write_table(df, layer, tabela, colunas_sql=None)` — grava um DataFrame como Parquet no MinIO (`df.to_parquet(..., storage_options=...)`) **e**, na mesma chamada, cadastra/atualiza a tabela no catálogo do Trino (`CREATE SCHEMA IF NOT EXISTS` + `DROP TABLE IF EXISTS` + `CREATE TABLE ... WITH (external_location = ..., format = 'PARQUET')`) — depois de rodar, `lh.query(...)` já enxerga. Os tipos de coluna são adivinhados a partir dos dtypes do pandas (int -> `BIGINT`, float -> `DOUBLE`, bool -> `BOOLEAN`, datetime64 -> `TIMESTAMP`, coluna de `datetime.date` -> `DATE`, resto -> `VARCHAR` — ver `_tipo_trino` no código-fonte); pra forçar tipos específicos, passe `colunas_sql="coluna TIPO, ..."` no mesmo formato de um `CREATE TABLE` do Trino (é o que `trino/init/construir_pipeline.py` faz pra bronze, em vez de confiar na inferência automática — ver abaixo). Decimal (comum em leituras via `lh.postgres()`) é convertido pra float automaticamente. Sempre sobrescreve por completo (`DROP` + `CREATE`), então rodar de novo depois de mudar o DataFrame nunca dá erro de "tabela já existe".
- `lh.drop_table(layer, tabela, apagar_arquivos=True)` — desfaz um `write_table`: tira do catálogo do Trino e, por padrão, apaga os arquivos Parquet do MinIO também (senão ficariam órfãos — o Trino esquece que existem, mas eles continuam ocupando espaço).
...
O `dbeaver/cloudbeaver.conf` (montado por cima do `cloudbeaver.conf` padrão da imagem) desliga o ícone de chat com IA do CloudBeaver — sem valor didático aqui, e evita que o aluno seja convidado a configurar uma API key de LLM só pra abrir o cliente SQL. Testado ao vivo via API (`{ serverConfig { enabledFeatures } }`): sem esse arquivo a lista vem `["ssh", "ai"]`; com ele, só `["ssh"]`.
...
`-v` também é o jeito de "religar" o `dbeaver/initial-data-sources.conf` **e** o `dbeaver/cloudbeaver.conf` se você editar algum dos dois depois da primeira subida — o CloudBeaver lê o `cloudbeaver.conf` do disco a cada start, mas grava um snapshot resolvido dele dentro do volume `dbeaver-workspace` (`.data/.cloudbeaver.runtime.conf`) já no primeiro start; testado ao vivo que, com esse snapshot presente, novas subidas ignoram o `cloudbeaver.conf` do disco e reusam o snapshot antigo — só um volume vazio garante que a versão atual do arquivo seja aplicada de verdade.
...
- Lembre que `dbeaver/initial-data-sources.conf` e `dbeaver/cloudbeaver.conf` só valem de verdade quando o volume `dbeaver-workspace` está vazio — editar qualquer um dos dois (por exemplo, pra adicionar uma terceira conexão, ou reverter o `enabledFeatures` que desliga o chat com IA) depois da primeira subida não tem efeito até rodar `docker compose down -v` (ou, pra resetar só o DBeaver-web sem apagar os dados do Postgres/MinIO: `docker compose stop dbeaver && docker compose rm -f dbeaver && docker volume rm data_lakehouse_101_dbeaver-workspace && docker compose up -d dbeaver`).
